...
const celsiusButton=$("celsius-button"),fahrenheitButton=$("fahrenheit-button"),themeToggle=$("theme-toggle"),favoriteButton=$("favorite-button"),favoritesList=$("favorites-list"),historyList=$("history-list"),clearHistoryButton=$("clear-history");
const precipitation=$("precipitation"),precipitationProbability=$("precipitation-probability"),visibility=$("visibility"),windDirection=$("wind-direction"),windGusts=$("wind-gusts"),sunrise=$("sunrise"),sunset=$("sunset"),uvIndex=$("uv-index"),weatherScene=$("weather-scene");

const weatherCodes={0:{description:"Clear sky",icon:"☀️",scene:"clear"},1:{description:"Mainly clear",icon:"🌤️",scene:"clear"},2:{description:"Partly cloudy",icon:"⛅",scene:"cloudy"},3:{description:"Overcast",icon:"☁️",scene:"cloudy"},45:{description:"Fog",icon:"🌫️",scene:"fog"},48:{description:"Depositing rime fog",icon:"🌫️",scene:"fog"},51:{description:"Light drizzle",icon:"🌦️",scene:"rain"},53:{description:"Moderate drizzle",icon:"🌦️",scene:"rain"},55:{description:"Dense drizzle",icon:"🌧️",scene:"rain"},56:{description:"Light freezing drizzle",icon:"🌧️",scene:"rain"},57:{description:"Dense freezing drizzle",icon:"🌧️",scene:"rain"},61:{description:"Slight rain",icon:"🌦️",scene:"rain"},63:{description:"Moderate rain",icon:"🌧️",scene:"rain"},65:{description:"Heavy rain",icon:"🌧️",scene:"rain"},66:{description:"Light freezing rain",icon:"🌧️",scene:"rain"},67:{description:"Heavy freezing rain",icon:"🌧️",scene:"rain"},71:{description:"Slight snow fall",icon:"🌨️",scene:"snow"},73:{description:"Moderate snow fall",icon:"🌨️",scene:"snow"},75:{description:"Heavy snow fall",icon:"❄️",scene:"snow"},77:{description:"Snow grains",icon:"❄️",scene:"snow"},80:{description:"Slight rain showers",icon:"🌦️",scene:"rain"},81:{description:"Moderate rain showers",icon:"🌧️",scene:"rain"},82:{description:"Violent rain showers",icon:"⛈️",scene:"storm"},85:{description:"Slight snow showers",icon:"🌨️",scene:"snow"},86:{description:"Heavy snow showers",icon:"❄️",scene:"snow"},95:{description:"Thunderstorm",icon:"⛈️",scene:"storm"},96:{description:"Thunderstorm with slight hail",icon:"⛈️",scene:"storm"},99:{description:"Thunderstorm with heavy hail",icon:"⛈️",scene:"storm"}};
...
function getSavedTheme(){try{return localStorage.getItem(THEME_STORAGE_KEY)==="dark"?"dark":"light"}catch{return"light"}}
...
function initializeApp(){
    updateFooterYear();updateUnitButtons();applyTheme(getSavedTheme());renderSavedCities();
    ...
    themeToggle.addEventListener("click",toggleTheme);favoriteButton.addEventListener("click",toggleFavorite);clearHistoryButton.addEventListener("click",clearHistory);
}
function setTemperatureUnit(unit){temperatureUnit=unit;try{localStorage.setItem(UNIT_STORAGE_KEY,unit)}catch{}updateUnitButtons();if(lastWeatherData){renderCurrentWeather(lastWeatherData.current);renderForecast(lastWeatherData.daily);renderExtendedDetails(lastWeatherData.current,lastWeatherData.daily,lastWeatherData.timezone)}}
...
function applyTheme(theme){const dark=theme==="dark";document.body.classList.toggle("dark-mode",dark);themeToggle.setAttribute("aria-pressed",String(dark));themeToggle.textContent=dark?"☀️ Light mode":"🌙 Dark mode";try{localStorage.setItem(THEME_STORAGE_KEY,dark?"dark":"light")}catch{}}
...
function renderWeather(location,data){lastLocation=location;lastWeatherData=data;const parts=[location.name,location.admin1,location.country].filter(Boolean);cityName.textContent=parts.join(", ");weatherDate.textContent=formatCurrentDate(data.current.time,data.timezone);renderCurrentWeather(data.current);renderExtendedDetails(data.current,data.daily,data.timezone);renderForecast(data.daily);updateWeatherScene(data.current);updateFavoriteButton()}
...
function renderExtendedDetails(current,daily,timezone){precipitation.textContent=`${Number(current.precipitation||0).toFixed(1)} mm`;precipitationProbability.textContent=`${Math.round(daily.precipitation_probability_max?.[0]??0)}%`;visibility.textContent=`${(Number(current.visibility||0)/1000).toFixed(1)} km`;windDirection.textContent=formatWindDirection(current.wind_direction_10m);windGusts.textContent=`${Math.round(current.wind_gusts_10m||0)} km/h`;sunrise.textContent=formatTime(daily.sunrise?.[0],timezone);sunset.textContent=formatTime(daily.sunset?.[0],timezone);uvIndex.textContent=Number(daily.uv_index_max?.[0]??0).toFixed(1)}
function updateWeatherScene(current){const info=getWeatherInfo(current.weather_code);weatherScene.classList.remove("weather-clear","weather-cloudy","weather-fog","weather-rain","weather-snow","weather-storm","weather-night");const scene=current.is_day===0?"night":info.scene;weatherScene.classList.add(`weather-${scene}`)}
function formatWindDirection(degrees){if(!Number.isFinite(Number(degrees)))return"--";const value=((Number(degrees)%360)+360)%360;const index=Math.round(value/22.5)%16;return`${compassPoints[index]} (${Math.round(value)}°)`}
function formatTime(dateTime,timezone){if(!dateTime)return"--";return new Intl.DateTimeFormat("en-US",{hour:"numeric",minute:"2-digit",timeZone:timezone}).format(new Date(dateTime))}
...
function getWeatherInfo(code){return weatherCodes[code]||{description:"Unknown",icon:"🌡️",scene:"cloudy"}}
...
function renderSavedCities(){renderList(favoritesList,getFavorites(),"No favorite cities yet.");renderList(historyList,getHistory(),"No recent searches yet.")}
function renderList(container,items,emptyText){container.innerHTML="";if(!items.length){const li=document.createElement("li");li.className="empty-list";li.textContent=emptyText;container.appendChild(li);return}items.forEach(item=>{const li=document.createElement("li");const button=document.createElement("button");button.type="button";button.className="history-item";button.textContent=[item.name,item.country].filter(Boolean).join(", ");button.addEventListener("click",()=>loadSavedCity(item));li.appendChild(button);container.appendChild(li)})}
...
function setControlsDisabled(disabled){searchButton.disabled=disabled;locationButton.disabled=disabled;celsiusButton.disabled=disabled;fahrenheitButton.disabled=disabled;favoriteButton.disabled=disabled;themeToggle.disabled=disabled}
